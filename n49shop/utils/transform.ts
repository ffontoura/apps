...
export function toProduct(
  ...
  const schemaProduct: Product = {
    ...
    // N49Shop description is returned as HTML and special characters and not working properly
...
function getStockVariant(stock: number | null | undefined): number {
  // N49Shop returns null when the stock is equal to infinity, so when null
...
function calculateProductMinPrice(product: ProductBaseN49Shop): number {
  const { variants } = product;
